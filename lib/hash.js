...
function keyedHashTogether(key, d1, d2) {
  return keyedHash(key, sjcl.bitArray.concat(keyedHash(key, d1), keyedHash(key, d2)));
}
exports.keyedHashTogether = keyedHashTogether;
...
function bindForRequest(rootSig, dischargeSig) {
  return sjcl.bitArray.equal(rootSig, dischargeSig) ? rootSig : keyedHashTogether(zeroKey, rootSig, dischargeSig);
}
