...
function getMacaroonPartsFromMacaroonBit(macBits) {
  ...
  while(curBits.length){
    ...
    if(bitsObj.nextBits.length == curBits.length) {
      throw new Error('Deserialization Error, bits lengths not changing');
    }
...
exports.deserialize = function (macHex) {
  ...
  var macaroonImport = getMacaroonPartsFromMacaroonBit(sjcl.codec.base64.toBits(macHex, true))
    .reduce(function (aggObj, macPartObj) {
      ...
      if(macPartObj.keyString === "vid"){ 
        aggObj.caveats[aggObj.caveats.length - 1].vid = sjcl.codec.base64.fromBits(macPartObj.valueBits);
        return aggObj;
      }
      if(macPartObj.keyString === "cl"){ 
        aggObj.caveats[aggObj.caveats.length - 1].cl = sjcl.codec.utf8String.fromBits(macPartObj.valueBits);
        return aggObj;  
      }
