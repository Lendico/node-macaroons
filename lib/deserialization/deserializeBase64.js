  ...
  var macaroonImport = getMacaroonPartsFromMacaroonBit(sjcl.codec.base64.toBits(macHex, true))
    .reduce(function (aggObj, macPartObj) {
      ...
      if(macPartObj.keyString === "vid"){ 
        macPartObj.caveats[macPartObj.caveats.length - 1].vid = sjcl.codec.utf8String.fromBits(macPartObj.valueBits);
        return aggObj;
      }
      if(macPartObj.keyString === "cl"){ 
        macPartObj.caveats[macPartObj.caveats.length - 1].cl = sjcl.codec.utf8String.fromBits(macPartObj.valueBits);
        return aggObj;  
      }
