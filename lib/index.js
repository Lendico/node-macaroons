...
Macaroon.prototype.bind = function(sig) {
  sig = bits.uint8ArrayToBitArray(sig);
  this._signature = hash.bindForRequest(sig, this._signature);
};
...
Macaroon.prototype.addThirdPartyCaveat = function(rootKey, caveatId, loc) {
  ...
  verificationId = bits.bitArrayToUint8Array(verificationId);
...
Macaroon.prototype.addCaveat = function(caveatId, verificationId, loc) {
  ...
  if (verificationId !== null) {
    ...
    verificationId = bits.uint8ArrayToBitArray(verificationId);
    cav._location = loc;
    cav._vid = verificationId;
  }
  this._caveats.push(cav);
  this._signature = hash.keyedHash2(this._signature, verificationId, sjcl.codec.utf8String.toBits(caveatId));
};
