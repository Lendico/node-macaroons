...
function Macaroon(data) {
  ...
  var _macaroon = {
    ...
    addFirstPartyCaveat: function(caveatId) { 
      ...
      _data._signature = hash.keyedHash2(_data._signature, null, sjcl.codec.utf8String.toBits(caveatId));
    },
    ...
    _verify: function(rootSig, rootKey, check, discharges, used) {
      ...
      var finalCaveatSignature = _data._caveats.reduce(function(updatedCaveatSig, cav) {
        if (cav._vid !== null) {
          var cavKey = encryption.decrypt(updatedCaveatSig, cav._vid);

          var dmCavObj = discharges.map(function (dm, di) { return {dm: dm, i: di}; })
            .find(function (dmObj) { return dmObj.dm.id() === cav._identifier; });

          if (!dmCavObj) { throw new Error('cannot find discharge macaroon for caveat ' + quote(cav._identifier)); }
          if (used[dmCavObj.i] !== 0) { throw new Error('discharge macaroon ' + quote(dmCavObj.dm.id()) + ' was used more than once '); }
          used[dmCavObj.i]++;

          dmCavObj.dm._verify(rootSig, encryption.decrypt(updatedCaveatSig, cav._vid), check, discharges, used);
        } else {
          var err = check(cav._identifier);
          if (err) {  throw new Error(err); }
        }

        return hash.keyedHash2(updatedCaveatSig, cav._vid, cav._identifier);
      }, caveatSig);
